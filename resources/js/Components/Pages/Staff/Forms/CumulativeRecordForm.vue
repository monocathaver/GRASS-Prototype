...
const getAllCumulativeRecordForms = async () => {
    try {
        const resp = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/v1/get-all-cumulative-record-forms`)
...
const assign = async (id) => {
    try {
        const resp = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/assign-form`, {
            assignee: id,
            form_name: 'Cumulative Record Form',
...
const goToInputs = () => {
    router.push({ name: 'staff-fieldCumulativeRecord' })
}

const goToRequests = () => {
    router.push({ name: 'staff-requestCumulativeRecord' })
}
